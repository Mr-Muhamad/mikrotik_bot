...
standalone(CallbackQueryHandler, pattern=PATTERNS["set_timeout"])(
    handle_timeout_selection
)
standalone(CallbackQueryHandler, pattern=PATTERNS["cancel_timeout"])(
    handle_timeout_selection
)

standalone(CallbackQueryHandler, pattern=PATTERNS["select_router"])(
    select_router_callback
)
...
standalone(CallbackQueryHandler, pattern=PATTERNS["menu_pdf_settings"])(
    pdf_settings_menu
)
...
standalone(CallbackQueryHandler, pattern=PATTERNS["discover_routers"])(
    discover_routers_callback
)
standalone(CallbackQueryHandler, pattern=PATTERNS["saved_routers"])(saved_routers_list)
standalone(CallbackQueryHandler, pattern=PATTERNS["saved_router"])(
    saved_router_selected
)
standalone(CallbackQueryHandler, pattern=PATTERNS["connect_router"])(connect_router)
standalone(CallbackQueryHandler, pattern=PATTERNS["delete_router"])(
    delete_router_confirm
)
standalone(CallbackQueryHandler, pattern=PATTERNS["confirm_delete_router"])(
    delete_router_execute
)
...
standalone(CallbackQueryHandler, pattern=PATTERNS["backup_restore"])(
    backup_restore_start
)
standalone(CallbackQueryHandler, pattern=PATTERNS["restore"])(backup_restore_select)
standalone(CallbackQueryHandler, pattern=PATTERNS["confirm_restore"])(
    backup_restore_confirm
)
standalone(CallbackQueryHandler, pattern=PATTERNS["userman_restore"])(
    userman_restore_start
)
standalone(CallbackQueryHandler, pattern=PATTERNS["userman_restore_tar"])(
    userman_restore_select
)
standalone(CallbackQueryHandler, pattern=PATTERNS["userman_restore_exec"])(
    userman_restore_execute
)
...
standalone(CallbackQueryHandler, pattern=PATTERNS["mark_payment"])(
    mark_batch_paid_handler
)
...
standalone(CallbackQueryHandler, pattern=PATTERNS["op_assign_router"])(
    op_assign_router_callback
)
standalone(CallbackQueryHandler, pattern=PATTERNS["op_revoke_router"])(
    op_revoke_router_callback
)
...
entry_point(CallbackQueryHandler, pattern=PATTERNS["hotspot_delete"])(
    hotspot_delete_start
)
entry_point(CallbackQueryHandler, pattern=PATTERNS["hotspot_search"])(
    hotspot_search_start
)
entry_point(CallbackQueryHandler, pattern=PATTERNS["hotspot_edit"])(hotspot_edit_start)
entry_point(CallbackQueryHandler, pattern=PATTERNS["hotspot_cards"])(
    hotspot_cards_start
)
entry_point(CallbackQueryHandler, pattern=PATTERNS["userman_cards"])(
    userman_cards_start
)
entry_point(CallbackQueryHandler, pattern=PATTERNS["disc_router"])(
    discovered_router_selected
)
entry_point(
    CallbackQueryHandler,
...
fallback(CallbackQueryHandler, pattern=PATTERNS["menu_userman"])(
    menu_userman_from_conversation
)
fallback(CallbackQueryHandler, pattern=PATTERNS["main_menu"])(end_conversation_to_main)
fallback(CallbackQueryHandler, pattern=PATTERNS["menu_hotspot"])(
    end_conversation_to_hotspot
)
fallback(CallbackQueryHandler, pattern=PATTERNS["menu_stats"])(
    end_conversation_to_stats
)
fallback(CallbackQueryHandler, pattern=PATTERNS["menu_backup"])(
    end_conversation_to_backup
)
fallback(CallbackQueryHandler, pattern=PATTERNS["menu_pdf_settings"])(
    end_conversation_to_pdf_settings
)
fallback(CallbackQueryHandler, pattern=PATTERNS["menu_routers"])(
    end_conversation_to_routers
)
fallback(CallbackQueryHandler, pattern=PATTERNS["menu_reports"])(
    end_conversation_to_reports
)
fallback(CallbackQueryHandler, pattern=PATTERNS["select_router"])(
    select_router_callback
)
fallback(CallbackQueryHandler, pattern=PATTERNS["menu_schedule"])(
    schedule_menu_from_conversation
)
...
state("WAITING_PASSWORD").callback(PATTERNS["add_back_to_username"])(
    add_back_to_username
)
...
state("WAITING_PROFILE").callback(PATTERNS["add_back_to_password"])(
    add_back_to_password
)
...
state("WAITING_BYTES_TOTAL").callback(PATTERNS["add_back_to_profile"])(
    add_back_to_profile
)
...
state("WAITING_UPTIME_VALUE").message(filters.TEXT & ~filters.COMMAND)(
    hotspot_add_uptime_value
)
state("WAITING_COMMENT").callback(PATTERNS["add_back_to_uptime"])(
    add_back_to_uptime_from_comment
)
...
# hotspot_delete flow
state("WAITING_DELETE_ID").message(filters.TEXT & ~filters.COMMAND)(
    hotspot_delete_search
)
state("WAITING_DELETE_SELECT").callback(PATTERNS["delete_user_star"])(
    hotspot_delete_select
)
state("WAITING_DELETE_SELECT").callback(PATTERNS["page_delete_user"])(
    handle_page_callback
)
state("WAITING_DELETE_SELECT").message(filters.TEXT & ~filters.COMMAND)(
    reprompt_select_user
)
...
state("WAITING_HOTSPOT_SEARCH").callback(PATTERNS["hotspot_search_page"])(hotspot_search_page_handler)
state("WAITING_HOTSPOT_SEARCH").callback(PATTERNS["host_kick_execute"])(
    hotspot_host_action
)
state("WAITING_HOTSPOT_SEARCH").callback(PATTERNS["host_reset_counters"])(
    hotspot_host_action
)
state("WAITING_HOTSPOT_SEARCH").callback(PATTERNS["host_toggle_disabled"])(
    hotspot_host_action
)
...
state("WAITING_HOTSPOT_SEARCH").message(filters.TEXT & ~filters.COMMAND)(
    hotspot_search_query
)

# userman_search flow
entry_point(CallbackQueryHandler, pattern=PATTERNS["userman_search"])(
    userman_search_start
)
...
state("WAITING_USERMAN_SEARCH").callback(PATTERNS["userman_search_page"])(userman_search_page_handler)
state("WAITING_USERMAN_SEARCH").callback(PATTERNS["um_kick_execute"])(
    userman_search_action
)
state("WAITING_USERMAN_SEARCH").callback(PATTERNS["um_reset_counters"])(
    userman_search_action
)
state("WAITING_USERMAN_SEARCH").callback(PATTERNS["um_toggle_disabled"])(
    userman_search_action
)
state("WAITING_USERMAN_SEARCH").callback(PATTERNS["um_delete"])(userman_search_action)
state("WAITING_USERMAN_SEARCH").callback(PATTERNS["um_add_profile"])(
    userman_search_add_profile
)
state("WAITING_USERMAN_SEARCH").callback(PATTERNS["um_profile"])(
    userman_search_add_profile_selected
)
state("WAITING_USERMAN_SEARCH").message(filters.TEXT & ~filters.COMMAND)(
    userman_search_query
)

# share_card flow — مشاركة كرت WiFi للعميل
state("WAITING_SHARE_RECIPIENT").message(filters.TEXT & ~filters.COMMAND)(
    share_card_send
)

# hotspot_edit flow
state("WAITING_EDIT_FIELD").message(filters.TEXT & ~filters.COMMAND)(
    hotspot_edit_search
)
...
state("WAITING_EDIT_VALUE").callback(PATTERNS["edit_back_to_fields"])(
    edit_back_to_fields
)
...
# hotspot_cards flow
state("WAITING_HOTSPOT_CARD_COUNT").message(filters.TEXT & ~filters.COMMAND)(
    hotspot_cards_count
)
state("WAITING_HOTSPOT_CARD_LENGTH").message(filters.TEXT & ~filters.COMMAND)(
    hotspot_cards_length
)
state("WAITING_HOTSPOT_CARD_PREFIX").callback(PATTERNS["hs_back_to_length"])(
    hs_back_to_length
)
state("WAITING_HOTSPOT_CARD_PREFIX").callback(PATTERNS["hs_skip_prefix"])(
    hotspot_cards_skip_prefix
)
state("WAITING_HOTSPOT_CARD_PREFIX").message(filters.TEXT & ~filters.COMMAND)(
    hotspot_cards_prefix
)
state("WAITING_HOTSPOT_CARD_TYPE").callback(PATTERNS["hs_card_type"])(
    hotspot_cards_type_selected
)
state("WAITING_HOTSPOT_CARD_PROFILE").callback(PATTERNS["hs_back_to_type"])(
    hs_back_to_type
)
state("WAITING_HOTSPOT_CARD_PROFILE").callback(PATTERNS["hs_back_to_profile"])(
    hs_back_to_profile
)
state("WAITING_HOTSPOT_CARD_PROFILE").callback(PATTERNS["hs_card_profile"])(
    hotspot_cards_profile_selected
)
state("WAITING_HOTSPOT_CARD_UPTIME").callback(PATTERNS["hs_back_to_profile"])(
    hs_back_to_profile
)
state("WAITING_HOTSPOT_CARD_UPTIME").callback(PATTERNS["hs_back_to_uptime"])(
    hs_back_to_uptime
)
state("WAITING_HOTSPOT_CARD_UPTIME").callback(PATTERNS["hs_skip_uptime"])(
    hotspot_cards_skip_uptime
)
state("WAITING_HOTSPOT_CARD_UPTIME").callback(PATTERNS["skip_uptime"])(
    hotspot_cards_skip_uptime_type
)
state("WAITING_HOTSPOT_CARD_UPTIME").callback(PATTERNS["uptime_type"])(
    hotspot_cards_uptime_type
)
state("WAITING_HOTSPOT_CARD_UPTIME").message(filters.TEXT & ~filters.COMMAND)(
    hotspot_cards_uptime_value
)
state("WAITING_HOTSPOT_CARD_BYTES").callback(PATTERNS["hs_back_to_uptime"])(
    hs_back_to_uptime
)
state("WAITING_HOTSPOT_CARD_BYTES").callback(PATTERNS["hs_skip_bytes"])(
    hotspot_cards_skip_bytes
)
state("WAITING_HOTSPOT_CARD_BYTES").message(filters.TEXT & ~filters.COMMAND)(
    hotspot_cards_bytes
)

# userman flow
state("WAITING_CARD_TYPE").callback(PATTERNS["card_type"])(userman_card_type_selected)
state("WAITING_CARD_TYPE").message(filters.TEXT & ~filters.COMMAND)(
    reprompt_card_type_text
)

state("WAITING_CARD_PROFILE").callback(PATTERNS["card_back_to_type"])(
    userman_back_to_type
)
state("WAITING_CARD_PROFILE").callback(PATTERNS["card_profile"])(
    userman_card_profile_selected
)
state("WAITING_CARD_PROFILE").message(filters.TEXT & ~filters.COMMAND)(
    reprompt_card_profile_text
)

state("WAITING_CARD_PAYMENT").callback(PATTERNS["card_back_to_profile"])(
    userman_back_to_profile
)
state("WAITING_CARD_PAYMENT").callback(PATTERNS["card_payment"])(
    userman_card_payment_selected
)

state("WAITING_CARD_MAC").callback(PATTERNS["card_back_to_payment"])(
    userman_back_to_payment
)
state("WAITING_CARD_MAC").callback(PATTERNS["card_mac_choice"])(
    userman_card_mac_selected
)

state("WAITING_CARD_PREFIX").callback(PATTERNS["card_back_to_mac"])(userman_back_to_mac)
state("WAITING_CARD_PREFIX").callback(PATTERNS["card_skip_prefix"])(
    userman_card_skip_prefix
)
state("WAITING_CARD_PREFIX").message(filters.TEXT & ~filters.COMMAND)(
    userman_card_prefix
)

state("WAITING_CARD_COUNT").callback(PATTERNS["card_back_to_prefix"])(
    userman_back_to_prefix
)
...
state("WAITING_DISC_USERNAME").message(filters.TEXT & ~filters.COMMAND)(
    disc_enter_username
)
state("WAITING_DISC_PASSWORD").message(filters.TEXT & ~filters.COMMAND)(
    disc_enter_password
)
...
state("WAITING_STATS_DAY").message(filters.TEXT & ~filters.COMMAND)(
    hotspot_stats_day_input
)
