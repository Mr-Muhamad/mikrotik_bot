...
async def discover_routers_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        await query.edit_message_text(DISCOVERY_PERMISSION_ERROR, reply_markup=get_router_keyboard())
...
async def discovered_router_selected(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if router_db and router_db.get("username"):
        ...
        await query.edit_message_text(f"{warning}{DISCOVERY_PASSWORD.format(ip)}", reply_markup=get_nav_back_keyboard())
        return WAITING_DISC_PASSWORD

    await query.edit_message_text(f"{warning}{DISCOVERY_CREDENTIALS.format(ip)}", reply_markup=get_nav_back_keyboard())
    ...
    try:
        ...
        if success:
            ...
            if is_update:
                ...
                router_id = await run_blocking(save_discovered_router,
                    ip=ip,
                    username=username,
                    password=password,
                    identity=identity,
            ...
            await run_blocking(log_action, "connect_discovered", ip, identity, update.effective_user.id)
            success_msg = (
                ROUTER_UPDATED.format(identity, version, ip)
                if is_update else
                DISCOVERY_SUCCESS.format(identity, version, ip)
            )
