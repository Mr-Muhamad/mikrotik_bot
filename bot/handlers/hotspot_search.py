...
async def hotspot_search_query(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await send_step(
        update,
        context,
        _format_search_results_text(paginator),
        get_search_results_keyboard(paginator, is_userman=False),
    )
    return WAITING_HOTSPOT_SEARCH
...
def _format_search_results_text(paginator: Paginator[RouterOSRow]) -> str:
    ...
    for i, h in enumerate(paginator.current_items):
        ...
        lines.append(detail)
    header = HOTSPOT_SEARCH_FOUND.format(count=len(paginator.items))
...
async def hotspot_search_back(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if on_host_detail and hosts is not None:
        ...
        await edit_clean(
            ...
            context,
            _format_search_results_text(paginator),
            get_search_results_keyboard(paginator),
        )
        return WAITING_HOTSPOT_SEARCH
...
async def hotspot_show_host(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        if idx < 0 or idx >= len(hosts):
            await safe_edit_plain(
                query, context, INVALID_SELECTION, reply_markup=get_hotspot_keyboard()
            )
        ...
        await safe_edit_plain(
            ...
            context,
            text,
            reply_markup=get_host_detail_keyboard(
...
async def hotspot_host_action(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        if idx is None:
            await safe_edit_plain(
                query, context, DEVICE_NOT_SELECTED, reply_markup=get_hotspot_keyboard()
            )
            cleanup_state(query.from_user.id, context.user_data)
            return ConversationHandler.END
        hosts = context.user_data.get("search_hosts", [])
        if idx < 0 or idx >= len(hosts):
            await safe_edit_plain(
                query, context, DEVICE_NOT_FOUND, reply_markup=get_hotspot_keyboard()
            )
            cleanup_state(query.from_user.id, context.user_data)
            return ConversationHandler.END
        host = hosts[idx]
        ...
        if success:
            await safe_edit_plain(
                query,
                context,
                HOTSPOT_KICK_SUCCESS_SINGLE.format(host_name=host_name),
                reply_markup=get_hotspot_keyboard(),
            )
        else:
            await safe_edit_plain(
...
async def block_mac_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if success:
        await safe_edit_plain(
            query,
            context,
            BLOCK_MAC_SUCCESS.format(mac=mac),
            reply_markup=get_hotspot_keyboard(),
        )
    else:
...
async def unblock_mac_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if success:
        await safe_edit_plain(
            query,
            context,
            UNBLOCK_MAC_SUCCESS.format(mac=mac),
            reply_markup=get_hotspot_keyboard(),
        )
    else:
