...
from .constants import WAITING_SEARCH
...
async def hotspot_search_query(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await send_step(update, context, _format_search_results_text(hosts), get_search_results_keyboard(hosts))
    return WAITING_SEARCH
...
@admin_only
async def hotspot_host_kick(update: Update, context: ContextTypes.DEFAULT_TYPE):
    query = update.callback_query
    await safe_answer_callback(query)
    router_key = None
    try:
        ...
        await send_error(update, context, e, router_key=router_key, log_extra="hotspot_host_kick", reply_markup=get_hotspot_keyboard())
    cleanup_state(query.from_user.id, context.user_data)
    return ConversationHandler.END
