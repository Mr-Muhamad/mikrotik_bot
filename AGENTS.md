...
| `/edit` (تعديل مستخدم) | يبحث عن المستخدم → يعرض قائمة الحقول القابلة للتعديل (الاسم/كلمة المرور/البروفايل/البايتات/الوقت/التعليق) → يُنفّذ التعديل على الراوتر → يعرض تأكيداً. يدعم kick وreset-counters وtoggle disable/enable. يتحقق من وجود المستخدم قبل أي كتابة (تغيير قيمة/تغيير بروفايل/تفعيل وتعطيل)؛ إن حُذف في الأثناء يعرض "المستخدم غير موجود أو تم حذفه مسبقاً" وينهي المحادثة. |
| `/delete` (حذف مستخدم) | يبحث عن المستخدم → يعرض تأكيداً بالبيانات → عند التأكيد يتحقق من وجود المستخدم مجدداً (حماية من الحذف المزدوج) ثم يحذف من الراوتر ويعمل `active-remove` للطرد. إن لم يكن موجوداً يعرض "المستخدم غير موجود أو تم حذفه مسبقاً" وينهي المحادثة. يُعيد `ConversationHandler.END`. |
