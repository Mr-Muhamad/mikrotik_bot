...
class TestUsermanCardTypeSelected:
    ...
    async def test_with_profiles_advances_to_profile(self):
        with patch("bot.handlers.userman.fetch_and_cache_profiles",
...
class TestUsermanCardCount:
    ...
    async def test_invalid_count_reprompts(self):
        ...
        with patch("bot.handlers.userman.validate_positive_int", return_value=(False, "Invalid")), \
    ...
    async def test_count_too_high_reprompts(self):
        ...
        with patch("bot.handlers.userman.validate_positive_int", return_value=(True, "")), \
             patch("bot.handlers.userman.send_step", new=AsyncMock()):
            result = await userman_card_count(update, _ctx())
        assert result == WAITING_CARD_COUNT
    ...
    async def test_success_sends_text_and_pdf(self):
        ...
        cards = [{"username": "111", "password": "222"}, {"username": "333", "password": "444"}]

        with patch("bot.handlers.userman.validate_positive_int", return_value=(True, "")), \
             patch("bot.handlers.userman.send_step", new=AsyncMock(return_value=status)), \
             patch("bot.handlers.userman.run_blocking", new=AsyncMock(side_effect=[cards, "/tmp/cards.pdf", None])), \
            ...
            mock_um.format_card = MagicMock(side_effect=lambda c, i: f"Card#{i+1}: {c['username']}")
    ...
    async def test_exception_ends(self):
        ...
        with patch("bot.handlers.userman.validate_positive_int", return_value=(True, "")), \
             ...
             patch("bot.handlers.userman.run_blocking", new=AsyncMock(side_effect=Exception("net down"))), \
    ...
    async def test_sends_secret_free_confirmation(self):
        ...
        with patch("bot.handlers.userman.validate_positive_int", return_value=(True, "")), \
             patch("bot.handlers.userman.send_step", new=AsyncMock(return_value=status)), \
             patch("bot.handlers.userman.run_blocking", new=AsyncMock(side_effect=[cards, "/tmp/x.pdf"])), \
             patch("bot.handlers.userman.userman_manager") as mock_um, \
...
class TestUsermanList:
    ...
    async def test_list_with_users(self):
        ...
        with patch("bot.handlers.userman.run_blocking", new=AsyncMock(return_value=users)), \
    ...
    async def test_list_exception(self):
        ...
        with patch("bot.handlers.userman.run_blocking", new=AsyncMock(side_effect=Exception("boom"))):
            await userman_list(update, ctx)
        update.callback_query.edit_message_text.assert_called_once()
...
class TestUsermanProfiles:
    ...
    async def test_profiles_with_data(self):
        ...
        with patch("bot.handlers.userman.run_blocking", new=AsyncMock(return_value=["1M", "2M"])):
    ...
    async def test_profiles_exception(self):
        ...
        with patch("bot.handlers.userman.run_blocking", new=AsyncMock(side_effect=Exception("net down"))):
            await userman_profiles(update, ctx)
        update.callback_query.edit_message_text.assert_called_once()
        call_kwargs = update.callback_query.edit_message_text.call_args.kwargs
        text = call_kwargs.get("text", "")
