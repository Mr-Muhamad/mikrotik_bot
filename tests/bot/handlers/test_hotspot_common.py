...
class TestSearchUsersForAction:
    ...
    async def test_no_router_ends(self):
        with patch("bot.handlers.hotspot_common.get_selected_router", return_value=None), \
    ...
    async def test_search_exception_ends(self):
        with patch("bot.handlers.hotspot_common.get_selected_router", return_value="discovered_1"), \
             patch("bot.handlers.hotspot_common.run_blocking", new=AsyncMock(side_effect=Exception("net down"))), \
    ...
    async def test_no_results_reprompts_for_delete(self):
        users = []
        with patch("bot.handlers.hotspot_common.get_selected_router", return_value="discovered_1"), \
             patch("bot.handlers.hotspot_common.run_blocking", new=AsyncMock(return_value=users)), \
    ...
    async def test_single_user_delete_goes_to_confirm(self):
        ...
        with patch("bot.handlers.hotspot_common.get_selected_router", return_value="discovered_1"), \
             patch("bot.handlers.hotspot_common.run_blocking", new=AsyncMock(return_value=users)), \
             patch("bot.handlers.hotspot_common.send_step", new=AsyncMock()) as mock_send:
            result = await search_users_for_action(_update(), _ctx(), "delete")
...
class TestExecuteAddUser:
    ...
    async def test_success_returns_true(self):
        mock_log = MagicMock()
        with patch("bot.handlers.hotspot_common.run_blocking", side_effect=[None, MagicMock()]), \
             patch("bot.handlers.hotspot_common.log_action", return_value=mock_log):
            ctx = _ctx()
            ctx.user_data["add_username"] = "u1"
            ctx.user_data["add_password"] = "p1"
    ...
    async def test_duplicate_error_clears_state(self):
        with patch("bot.handlers.hotspot_common.run_blocking", new=AsyncMock(side_effect=Exception("already have user with this name"))):
            ctx = _ctx()
            ctx.user_data["add_username"] = "u1"
            ctx.user_data["add_password"] = "p1"
            ctx.user_data["add_profile"] = "1M"
            ctx.user_data["add_bytes"] = "1G"
            ctx.user_data["add_uptime"] = "1d"
            ctx.user_data["uptime_unit"] = "d"
            ok, err = await execute_add_user(ctx, ADMIN_ID, "discovered_1", "test")
        assert ok is False
        assert err == "duplicate"
        assert "add_username" not in ctx.user_data
        assert "add_bytes" not in ctx.user_data

    @pytest.mark.asyncio
    async def test_other_error_returns_message(self):
        with patch("bot.handlers.hotspot_common.run_blocking", new=AsyncMock(side_effect=Exception("connection timeout"))):
            ctx = _ctx()
            ctx.user_data["add_username"] = "u1"
            ctx.user_data["add_password"] = "p1"
            ctx.user_data["add_profile"] = "1M"
    ...
    async def test_optional_fields_use_defaults(self):
        ...
        with patch("bot.handlers.hotspot_common.run_blocking", side_effect=mock_run), \
