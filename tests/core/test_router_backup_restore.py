...
class TestListRouterBackups:
    ...
    def test_lists_system_backups(self, mock_api):  # type: ignore[reportMissingParameterType]
    ...
    @patch("core.backup.restore.mikrotik_api")
    def test_sorted_descending(self, mock_api):  # type: ignore[reportMissingParameterType]
        mock_api.execute.return_value = [
        ...
        assert names == sorted(names, reverse=True)  # type: ignore[reportArgumentType]

    @patch("core.backup.restore.mikrotik_api")
    def test_empty_on_exception(self, mock_api):  # type: ignore[reportMissingParameterType]
        mock_api.execute.side_effect = Exception("fail")
        result = self.restorer.list_router_backups(self.router_key)
        assert result == []

    @patch("core.backup.restore.mikrotik_api")
    def test_empty_list(self, mock_api):  # type: ignore[reportMissingParameterType]
        mock_api.execute.return_value = []
        result = self.restorer.list_router_backups(self.router_key)
        assert result == []
...
class TestRestoreBackup:
    ...
    @patch("core.backup.restore.mikrotik_api")
    def test_restore_system_backup(self, mock_api):  # type: ignore[reportMissingParameterType]
    ...
    @patch("core.backup.restore.mikrotik_api")
    def test_restore_export_backup(self, mock_api):  # type: ignore[reportMissingParameterType]
        mock_api.get_router_name.return_value = "Router1"
        result = self.restorer.restore_backup(self.router_key, "export_2026.rsc")
        assert result["success"] is True

    @patch("core.backup.restore.mikrotik_api")
    def test_invalid_filename_rejected(self, mock_api):  # type: ignore[reportMissingParameterType]
        mock_api.get_router_name.return_value = "Router1"
    ...
    @patch("core.backup.restore.mikrotik_api")
    def test_unknown_extension(self, mock_api, _mock_validate):  # type: ignore[reportMissingParameterType]
        mock_api.get_router_name.return_value = "Router1"
        result = self.restorer.restore_backup(self.router_key, "backup_2026.zip")
        assert result["success"] is False

    @patch("core.backup.restore.mikrotik_api")
    def test_exception_returns_error(self, mock_api):  # type: ignore[reportMissingParameterType]
        mock_api.get_router_name.return_value = "Router1"
