...
class TestBackupSingleRouter:
    ...
    async def test_unhealthy_router_skipped(self, mock_run):
        mock_run.return_value = (False, "offline")
        scheduler = BackupScheduler()
        failed = []
        successful = []
        await scheduler._backup_single_router(
    ...
    @pytest.mark.asyncio
    @patch("core.backup_scheduler.run_blocking", new_callable=AsyncMock)
    async def test_userman_error_records_failure(self, mock_run):
    ...
    @pytest.mark.asyncio
    @patch("core.backup_scheduler.run_blocking", new_callable=AsyncMock)
    async def test_full_backup_fails(self, mock_run):
        from librouteros.exceptions import LibRouterosError
...
class TestDoExpiryCheck:
    @pytest.mark.asyncio
    @patch("core.backup_scheduler.run_blocking", new_callable=AsyncMock)
    async def test_sends_expiry_alerts(self, mock_run):
        ...
        scheduler = BackupScheduler()
        await scheduler._do_expiry_check(mock_context)
        mock_context.bot.send_message.assert_called()

    @pytest.mark.asyncio
    @patch("core.backup_scheduler.run_blocking", new_callable=AsyncMock)
    async def test_skips_when_no_expiring(self, mock_run):
        mock_context = MagicMock()
        mock_context.bot = AsyncMock()
        mock_run.side_effect = [
            [{"id": 1, "username": "admin", "identity": "R1"}],
    ...
    @pytest.mark.asyncio
    @patch("core.backup_scheduler.run_blocking", new_callable=AsyncMock)
    async def test_skips_router_without_username(self, mock_run):
        mock_context = MagicMock()
        mock_context.bot = AsyncMock()
        mock_run.side_effect = [
    ...
    @pytest.mark.asyncio
    @patch("core.backup_scheduler.run_blocking", new_callable=AsyncMock)
    async def test_exception_in_get_expiring(self, mock_run):
        mock_context = MagicMock()
        mock_context.bot = AsyncMock()
        mock_run.side_effect = [
            [{"id": 1, "username": "admin", "identity": "R1"}],
    ...
    @pytest.mark.asyncio
    @patch("core.backup_scheduler.run_blocking", new_callable=AsyncMock)
    async def test_admin_send_fails(self, mock_run):
        mock_context = MagicMock()
        mock_context.bot = AsyncMock()
...
class TestDoStatsSnapshot:
    @pytest.mark.asyncio
    @patch("core.backup_scheduler.run_blocking", new_callable=AsyncMock)
    async def test_saves_snapshot(self, mock_run):
        mock_context = MagicMock()
        mock_run.side_effect = [
            [{"id": 1, "username": "admin", "identity": "R1"}],
        ...
        scheduler = BackupScheduler()
        await scheduler._do_stats_snapshot(mock_context)
        assert mock_run.call_count == 3

    @pytest.mark.asyncio
    @patch("core.backup_scheduler.run_blocking", new_callable=AsyncMock)
    async def test_skips_when_no_stats(self, mock_run):
        mock_context = MagicMock()
        mock_run.side_effect = [
            [{"id": 1, "username": "admin", "identity": "R1"}],
            None,
    ...
    @pytest.mark.asyncio
    @patch("core.backup_scheduler.run_blocking", new_callable=AsyncMock)
    async def test_exception_in_stats(self, mock_run):
        mock_context = MagicMock()
        mock_run.side_effect = [
            [{"id": 1, "username": "admin", "identity": "R1"}],
            Exception("timeout"),
...
class TestDoBackupNotifications:
    ...
    @patch("core.backup_scheduler.run_blocking", new_callable=AsyncMock)
    async def test_sends_failure_notification(self, mock_run):
        mock_context = MagicMock()
        mock_context.bot = AsyncMock()
        mock_run.side_effect = [
            [{"id": 1, "username": "admin", "identity": "R1"}],
    ...
    @patch("config.ADMIN_IDS", [724730774])
    @patch("core.backup_scheduler.run_blocking", new_callable=AsyncMock)
    async def test_admin_send_failure_graceful(self, mock_run):
        mock_context = MagicMock()
        mock_context.bot = AsyncMock()
        mock_context.bot.send_message.side_effect = OSError("tg down")
        mock_run.side_effect = [
...
class TestStopRemovesAllJobs:
    def test_stop_removes_expiry_and_snapshot_jobs(self, mock_job_queue):
