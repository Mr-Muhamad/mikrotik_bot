...
def test_scan_file_flags_hardcoded_userman(tmp_path):  # type: ignore[reportMissingParameterType]
    bad = tmp_path / "bad.py"
...
def test_scan_file_flags_hardcoded_v6_userman(tmp_path):  # type: ignore[reportMissingParameterType]
    bad = tmp_path / "bad.py"
    bad.write_text(
...
def test_scan_file_allows_central_helper_pattern(tmp_path):  # type: ignore[reportMissingParameterType]
    ok = tmp_path / "ok.py"
    ok.write_text(
